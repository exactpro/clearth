...
public class DefaultClearThConnectionStorage implements ClearThConnectionStorage
{
	...
	@Override
	public void registerType(ConnectionTypeInfo info) throws SettingsException, ConnectivityException
	{
		checkTypeBeforeRegister(info);
		try
		{
			Files.createDirectories(info.getDirectory());
			typeInfoMap.put(info.getName(), info);
		}
		catch (IOException e)
		{
		...
		try
		{
			settingsModels.put(info.getName(), settingsProcessor.process(info.getSettingsClass()));
		}
		catch (Exception e)
		{
			throw new ConnectivityException("Error while processing connection settings class", e);
		}
	}

	private void checkTypeBeforeRegister(ConnectionTypeInfo suspect) throws SettingsException
	{
		if (typeInfoMap.containsKey(suspect.getName()))
			throw new SettingsException("Connection type '"+suspect.getName()+"' is already registered");
		...
		for (ConnectionTypeInfo typeInfoInMap : typeInfoMap.values())
		{
			if (typeInfoInMap.getDirectory().equals(suspect.getDirectory()))
				throw new SettingsException(format("Connection type '%s' cannot be registered, " +
						...
						suspect.getName(), typeInfoInMap.getName(), suspect.getDirectory().toAbsolutePath()));
		}
	}
	...
	public ClearThConnection getConnection(String name, String type)
	{
		...
		return ((connection != null) && StringUtils.equals(type, connection.getTypeInfo().getName()))
				? connection : null;
	}
	...
	public ClearThConnection createConnection(String type) throws ConnectivityException
	{
		...
		catch (InstantiationException | IllegalAccessException e)
		{
			throw new ConnectivityException(e, "Cannot create connection of class '%s' with type '%s'",
					info.getConnectionClass(), info.getName());
		}
	...
	protected void checkConnectionType(String connectionName, ConnectionTypeInfo info) throws ConnectivityException
	{
		...
		if (!typeInfoMap.containsKey(info.getName()))
			throw new ConnectivityException("Type '{}' of connection '{}' is not registered in storage.",
					info.getName(), connectionName);
	...
	protected void validateName(String connectionName) throws SettingsException
	{
		...
		if (connection != null)
		{
			throw new SettingsException(format("Connection with name '%s' already exists and has type '%s'.",
					connectionName, connection.getTypeInfo().getName()));
		}
	...
	protected Processor createSettingsProcessor() throws ConnectivityException
	{
		try
		{
